    ...
    <div className="space-y-8 pb-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-white rounded-[12px] border border-[#D6DEE6] shadow-sm overflow-hidden">
          <div className="p-6 border-b border-[#D6DEE6] bg-[#F5F7FA] flex items-center gap-2">
            <TrendingDown className="w-5 h-5 text-[#EB5757]" />
            <h3 className="font-bold text-[#1F3A5F] uppercase tracking-tight">Top 15 Pérdidas Críticas</h3>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead>
                <tr className="bg-[#A7C4E0]">
                  ...
                  <th className="px-4 py-3 text-[10px] font-bold text-[#1F3A5F] uppercase tracking-widest text-center">Unidad</th>
                  <th className="px-4 py-3 text-[10px] font-bold text-[#1F3A5F] uppercase tracking-widest text-right">Variación</th>
                  <th className="px-4 py-3 text-[10px] font-bold text-[#1F3A5F] uppercase tracking-widest text-right">Precio Unit.</th>
                  <th className="px-4 py-3 text-[10px] font-bold text-[#1F3A5F] uppercase tracking-widest text-right">Impacto $</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[#D6DEE6]">
                {topLossesProducts.map((p) => (
                  <tr key={p.articulo + p.cc + p.sede} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-center text-[10px] font-bold text-slate-500">{p.subarticulo}</td>
                    <td className="px-4 py-3 text-right text-rose-600 font-bold text-xs">
                      {p.totalDiferencia.toFixed(2)}
                    </td>
                    <td className="px-4 py-3 text-right text-[10px] font-bold text-slate-600">
                      {formatCurrency(p.ultimoCoste || p.costePromedio)}
                    </td>
        ...
        <div className="bg-white rounded-[12px] border border-[#D6DEE6] shadow-sm overflow-hidden">
          <div className="p-6 border-b border-[#D6DEE6] bg-[#F5F7FA] flex items-center gap-2">
            ...
            <h3 className="font-bold text-[#1F3A5F] uppercase tracking-tight">Top 15 Ganancias (Sobrantes)</h3>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead>
      ...
      <div className="space-y-4">
        <div className="bg-[#0F2044] text-white p-6 rounded-[12px] shadow-lg relative overflow-hidden flex items-center justify-between">
          <div>
            <h2 className="text-xl font-bold uppercase tracking-tight">
              Ranking de Riesgo Operativo {selectedSede ? `— ${selectedSede}` : ''}
            </h2>
            <p className="text-[#A7C4E0] text-sm font-medium mt-1">
              Artículos con mayor criticidad — valorización de impacto económico
            </p>
          </div>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-[#0F2044]">
                  <th className="px-4 py-3 text-[10px] font-bold text-white uppercase tracking-widest">Pos</th>
                  <th className="px-4 py-3 text-[10px] font-bold text-white uppercase tracking-widest">Sede</th>
                  <th className="px-4 py-3 text-[10px] font-bold text-white uppercase tracking-widest">Artículo</th>
                  <th className="px-4 py-3 text-[10px] font-bold text-white uppercase tracking-widest text-center">Unidad</th>
                  <th className="px-4 py-3 text-[10px] font-bold text-[#C4973A] uppercase tracking-widest text-right">Precio Unit.</th>
                  <th className="px-4 py-3 text-[10px] font-bold text-white uppercase tracking-widest text-right">Variación</th>
                  <th className="px-4 py-3 text-[10px] font-bold text-[#C4973A] uppercase tracking-widest text-right">Impacto $</th>
                  <th className="px-4 py-3 text-[10px] font-bold text-white uppercase tracking-widest text-center">Puntaje</th>
                  <th className="px-4 py-3 text-[10px] font-bold text-white uppercase tracking-widest">Riesgo</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[#D6DEE6]">
                {problematicProducts.map((p, idx) => (
                  <tr key={p.articulo + idx} className={idx % 2 === 0 ? 'bg-white hover:bg-slate-50' : 'bg-[#F5F8FC] hover:bg-[#E8EFF8]'} style={{ transition: 'background 0.15s' }}>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3 text-[11px] font-bold text-slate-500">{p.sede}</td>
                    <td className="px-4 py-3">
                      <div className="flex flex-col">
                        <span className="font-bold text-[#0F2044] text-xs">{p.articulo}</span>
                        <span className="text-[9px] text-slate-400 uppercase font-bold">{p.cc || 'SIN CC'}</span>
                      </div>
                    </td>
                    <td className="px-4 py-3 text-center text-[10px] font-bold text-slate-500">{p.subarticulo}</td>
                    <td className="px-4 py-3 text-right text-[11px] font-bold text-[#1B4F8A]">
                      {formatCurrency(p.ultimoCoste || p.costePromedio)}
                    </td>
                    <td className="px-4 py-3 text-right text-xs font-bold text-rose-700">
                      {p.variacionAbs?.toFixed(2) ?? '—'}
                    </td>
                    <td className="px-4 py-3 text-right font-bold text-rose-600 text-sm">
                      {formatCurrency(p.impacto)}
                    </td>
                    <td className="px-4 py-3 text-center font-bold text-slate-700">{Math.round(p.score)}</td>
                    <td className="px-4 py-3">
                      <div className="inline-flex items-center gap-1 px-2 py-1 rounded-[4px] text-[10px] font-bold uppercase"
              ...
              <tfoot>
                <tr className="bg-[#F0F3F7] border-t-2 border-[#0F2044]">
                  <td colSpan={6} className="px-4 py-3 text-xs font-bold text-[#0F2044] uppercase tracking-wider text-right">Total Impacto:</td>
                  <td className="px-4 py-3 text-right font-bold text-rose-600 text-sm">
                    {formatCurrency(problematicProducts.reduce((acc, p) => acc + p.impacto, 0))}
                  </td>
                  <td colSpan={2}></td>
                </tr>
              </tfoot>
