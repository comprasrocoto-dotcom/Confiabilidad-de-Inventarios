...
const ALIASES: Record<string, string[]> = {
  ...
  "costeLinea": ["coste linea", "costo linea", "coste", "costo", "valor costo", "coste linea"],
  "variacion": ["variacion stock", "variacion", "ajuste", "ajuste stock", "diferencia stock", "movimiento stock", "variacion stock"],
  "subfamilia": ["subfamilia", "familia"],
  "stockFecha": ["stock a fecha", "stock fecha"],
  "stockInventario": ["stock inventario", "inventario"],
  "codBarras": ["cod barras", "codigo barras", "cod.", "barras", "cod barras"]
};
...
export function normalizeData(rawRows: RawInventoryRow[]): { articles: ArticleSummary[], errors: string[], debug: any } {
  ...
  normalized.forEach(row => {
    ...
    if (typeof stockFechaRaw === 'number') {
      stockFechaVal = stockFechaRaw;
    } else if (typeof stockFechaRaw === 'string') {
      stockFechaVal = parseFloat(stockFechaRaw.replace(/,/g, '').trim()) || 0;
    }
  ...
  const articles: ArticleSummary[] = Array.from(grouped.values()).map(summary => {
    ...
    // Stock a fecha (último movimiento)
    const ultimoStock = summary.movements.length > 0
    ...
    let margenAplicado = 0;
    let margenTipo: ArticleSummary['margenTipo'] = 'FIJO';
    let margenPct = 0;
    ...
    summary.margenTipo = margenTipo;
    summary.margenPct = margenPct;
